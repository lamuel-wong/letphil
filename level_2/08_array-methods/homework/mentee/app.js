...
const hasPriorityOrder = orders.some(function (order) {
  return order.isPriority === true;
});
console.log("Has priority order: " + hasPriorityOrder);

const hasOver300 = orders.some(function (order) {
  return order.total > 300;
});
console.log("Has $300+ order: " + hasOver300);

const allDeliveredOrPending = orders.every(function (order) {
  return order.status === "delivered" || order.status === "pending";
});
console.log("All active: " + allDeliveredOrPending);

const allPriorityMultiItem = orders
  .filter(function (order) {
    return order.isPriority === true;
  })
...
priorityDisplay.forEach(function (display) {
  console.log(display);
});
